...
async function schedulePost() {
  ...
  // FIXED: Convert local Egypt time to UTC for GitHub Actions
  ...
  const localTimeStr = localDateTime.toLocaleString('en-US', {
    month: 'short',
    day: 'numeric',
    hour: '2-digit',
    minute: '2-digit'
  });
  alert(`✅ Post scheduled for ${localTimeStr} (your local time)`);
}
...
function editPost(id) {
  ...
  // Convert UTC back to local time for editing
...
async function saveEditedPost() {
  ...
  // FIXED: Convert local Egypt time to UTC
  const localDateTime = new Date(`${date}T${time}:00`);
  post.scheduledTime = localDateTime.toISOString();
